...
public class UserService {
    ...
    public UserPage findPage(String query, int page) {
        String normaliedQuery = query == null ? "" : query.trim();
        List<UserForm> filtered = findAll().stream()
                .filter(user -> matchesQuery(user, normaliedQuery))
                .toList();

        int safePage = Math.max(page, 1); // safePage không bao giờ nhỏ hơn 1
        ...
        int fromIndex = (safePage - 1) * pageSize; // tính vị trí bắt đầu lấy dữ liệu của một trang
        int toIndex = Math.min(fromIndex + pageSize, totalItems); // tính vị trí kết thúc lấy dữ liệu của trang
        List<UserForm> pageItems = fromIndex >= totalItems
                ? List.of()
                : filtered.subList(fromIndex, toIndex);

        return new UserPage(pageItems, normaliedQuery, safePage, pageSize, totalItems, totalPages);
    }
