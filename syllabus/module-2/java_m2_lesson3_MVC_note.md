...
### Bài học tham khảo
- [Bài 3: Mô hình MVC — Model, View, Controller — Github module 2](https://github.com/nguyenvudangkhoa189/t3h-ltv-java-module-2/blob/dev/syllabus/module-2/java_m2_bai3_MVC.md)
...
- Vẽ sơ đồ trên web draw.io
- Hình sơ đồ trong: ex-lesson3/Module2-lesson3-ex1.png
- File dùng để import trên web draw.io: ex-lesson3/Module2-lesson3-ex1.drawio
...
| Render template `orders/detail.html` | DispatcherServlet -> wrong<br/>View (Thymeleaf) — được Controller chọn -> correct |
...
| Bước | Mô tả | Layer                                      |
|------|-------|--------------------------------------------|
| 1 | Trình duyệt gửi `POST /orders` kèm form data | Browser -> wrong<br/>Controller -> correct |
| 2 | Kiểm tra tồn kho đủ trước khi tạo đơn | Service                                    |
| 3 | `INSERT INTO orders (...)` | Repository                                 |
| 4 | `return "orders/success"` | Controller                                 |
| 5 | Hiển thị trang "Đặt hàng thành công" | View (Thymeleaf)                           |
...
- View: 
  ...
  - nhận dữ liệu từ Controller và hiển thị lên giao diện
  - gửi thao tác của người dùng về Controller
- Controller: 
  - là trung gian giữa View và Model (thông qua Service)
  - điều khiển luồng chương trình: nhận input -> gọi xử lý -> trả output
  - không chứa logic nghiệp vụ phức tạp - chỉ điều phối
...
- Vì sẽ vi phạm nguyên tắc hân tách trách nhiệm (Separation of Concerns) của mô hình MVC. 
- Khó bảo trì:
  - Sau này chuyển sang MongoDB:
    - Nếu View tự truy vấn Database, phải sửa tất cả các View.
    - Nếu dùng Repository, chỉ cần sửa Repository, còn View không thay đổi.
- Khó kiểm thử:
  - Có thể mock userService để kiểm thử Controller.
  - Nếu View tự truy vấn Database thì muốn test View phải có Database thật, việc kiểm thử trở nên phức tạp.
- Bảo mật:
  - Điều này có thể làm lộ: chuỗi kết nối Database, tài khoản và mật khẩu Database, logic truy vấn.
  - Trong Spring MVC, các thông tin này chỉ nên nằm ở tầng Repository/Service.
- Kết luận: View không kết nối Database trực tiếp để đảm bảo mã nguồn dễ bảo trì, dễ kiểm thử, bảo mật hơn và tuân thủ đúng kiến trúc MVC.

**3. Service layer giải quyết vấn đề gì mà Controller không nên làm trực tiếp?**
- Xử lý business logic thay vì để Controller xử lý.
- Tách biệt trách nhiệm giữa các tầng (Separation of Concerns).
- Có thể tái sử dụng nghiệp vụ cho nhiều Controller hoặc API.
- Dễ bảo trì và kiểm thử hơn.

**4. Khác biệt giữa **MVC pattern** và **Spring MVC**?**

| | MVC (pattern) | String MVC (framework) |
|-|---------------|------------------------|
| Bản chất | Mô hình thiết kế/ kiến trúc phần mềm | Module của String Framework triển khai pattern MVC |
| Phạm vi | Khái niệm chung, áp dụng nhiều ngôn ngữ | Cụ thể cho Java/ Spring  |
| Thành phần | Model, View, Controller | DispatcherServlet, @Controller, ViewResolver, Thymeleaf, … |
| Quan hệ | Ý tưởng thiết kế | Công cụ thực thi ý tưởng đó |
...
| MVC Model layer | Spring Model object (model.addAttribute) |
|---------------|------------------------|
| Là tầng Model trong kiến trúc MVC. | Là đối tượng của Spring MVC dùng để truyền dữ liệu từ Controller sang View. |
| Chứa dữ liệu và nghiệp vụ (Entity, DTO, Service, Repository...). | Chỉ chứa các thuộc tính (attributes) để View (Thymeleaf) hiển thị.  |
| Tham gia xử lý nghiệp vụ của ứng dụng. | Không xử lý nghiệp vụ, chỉ đóng vai trò "mang dữ liệu". |
| Ví dụ: User, Book, Loan, UserService, UserRepository. | Ví dụ: model.addAttribute("users", users); |
- Kết luận:
  - MVC Model layer = tầng dữ liệu và nghiệp vụ của ứng dụng.
  - Spring Model object = "chiếc hộp" dùng để truyền dữ liệu từ Controller → View (Thymeleaf).

**6. DTO là gì? Khác Entity ở điểm nào?**
- DTO là class dùng để trao đổi dữ liệu giữa các layer hoặc qua API - tách khỏi Entity để không lộ cấu trúc database ra ngoài.
- Điểm khác giữa DTO vs Entity:

| DTO | Entity |
|-----|--------|
| Dùng để truyền dữ liệu. | Đại diện cho dữ liệu trong cơ sở dữ liệu. |
| Không ánh xạ trực tiếp với bảng trong DB. | Được ánh xạ với bảng trong DB (ví dụ qua JPA @Entity). |
| Có thể chỉ chứa một phần dữ liệu cần thiết. | Thường chứa đầy đủ các cột của bảng. |
| Dùng ở Controller, View, API. | Dùng ở Repository và Service để lưu/truy vấn dữ liệu. |
- Kết luận:
  - Entity: biểu diễn dữ liệu trong Database.
  - DTO: dùng để truyền dữ liệu giữa các tầng hoặc giữa client và server. Thường Controller nhận DTO, Service chuyển DTO thành Entity để lưu vào Database.

**7. Khi nào dùng `@Controller` và khi nào dùng `@RestController`? *(gợi ý: xem Bài 5)***
- Khi nào dùng `@Controller`:
  - Dùng khi xây dựng ứng dụng MVC truyền thống (Server-side Rendering), trả về View (Thymeleaf, JSP...).
  - Trả về tên View.
  - Dữ liệu được truyền sang View bằng Model.
- Khi nào dùng `@RestController`:
  - Dùng khi xây dựng REST API, trả về dữ liệu (JSON, XML...), không trả về View.
  - Mỗi phương thức mặc định có @ResponseBody.
  - Thường dùng cho Frontend React, Angular, Mobile App,...
- So sánh `@Controller` vs `@RestController`:

| `@Controller` | `@RestController` |
|-----|--------|
| Trả về View (Thymeleaf, JSP...) | Trả về dữ liệu (JSON/XML) |
| Dùng cho MVC truyền thống | Dùng cho REST API |
| Thường kết hợp với Model | Không cần Model để render View |
| Có thể dùng @ResponseBody nếu muốn trả JSON | Tự động có @ResponseBody |

**8. Nêu 2 ưu điểm và 2 hạn chế của MVC.**
- Ưu điểm của MVC:
  - Phân tách trách nhiệm (Separation of Concerns)
    - Model, View và Controller có nhiệm vụ riêng biệt.
    - Mã nguồn dễ đọc, dễ bảo trì và dễ mở rộng.
  - Dễ tái sử dụng và kiểm thử
    - Business logic nằm trong Service/Model nên có thể dùng lại cho nhiều giao diện (Web, REST API, Mobile).
    - Dễ viết Unit Test cho từng tầng.
- Hạn chế của MVC:
  - Cấu trúc phức tạp hơn
    - Một chức năng phải chia thành nhiều lớp (Controller, Service, Repository, View...), tạo nhiều file hơn so với ứng dụng nhỏ.
  - Khó học đối với người mới
    - Cần hiểu cách các tầng phối hợp với nhau và luồng xử lý Request → Response, nên mất thời gian làm quen.

**9. Fat Controller là gì? Làm sao tránh?**
- Fat Controller là Controller chứa quá nhiều business logic và xử lý dữ liệu.
- Cách tránh:
  - Chỉ để Controller nhận request và trả response.
  - Đưa business logic sang Service.
  - Để Repository đảm nhiệm việc truy cập Database.
